...
type ManualCommandTask = 'code_gen' | 'bug_fix' | 'refactoring' | 'perf_opt' | 'test_gen' | 'project_explain';

interface CommandSpec {
  commandId: string;
  task: ManualCommandTask;
  defaultInstruction: string;
  useRagDefault: boolean;
  mode: 'insertBelow' | 'replaceSelection' | 'showExplanation';
}
...
export function registerCommands(context: vscode.ExtensionContext): void {
  for (const spec of COMMANDS) {
    context.subscriptions.push(
      vscode.commands.registerCommand(spec.commandId, async () => runCommand(spec))
    );
  }
  context.subscriptions.push(output);
}

async function runCommand(spec: CommandSpec): Promise<void> {
  ...
  try {
    ...
    vscode.window.showErrorMessage(`AI Code Assistant: ${(error as Error).message}`);
    return;
  }
...
async function handleResponse(
  ...
  if (response.validation && response.validation.valid === false) {
    vscode.window.showWarningMessage(
      `AI Code Assistant: inserted output, but backend validation reported warnings: ${response.validation.warnings.join(' ')}`
    );
  }
}

function showExplanation(response: GenerateResponse): void {
  output.clear();
  ...
  if (response.rag_sources.length > 0) {
    ...
    for (const source of response.rag_sources) {
      output.appendLine(
        `- ${source.file_path ?? 'unknown'}:${source.start_line ?? '?'}-${source.end_line ?? '?'} ` +
